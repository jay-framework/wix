...
const { createAppProject, getAppProject } = wixRequire(
  '@wix/ambassador-velo-backend-v1-app-project/http',
);
const { getOrCreateCompanionApp } = wixRequire(
  '@wix/ambassador-devcenter-apps-companionapps-v1-companion-app/http',
);
...
async function main() {
  ...
  fs.writeFileSync(path.join(wixDir, 'project-info.json'), JSON.stringify(projectData, null, 2));
