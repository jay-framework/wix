...
if (args.includes("--help") || args.includes("-h")) {
  console.log(
    `${colors.yellow}Jay Framework Package Sync Script${colors.reset}`,
  );
  console.log("");
  console.log("Usage:");
  ...
  console.log(
    "                    Default: ../jay (relative to script location)",
  );
  console.log("");
...
function copyPackageArtifacts(sourcePath, targetPath) {
  ...
  const jayCommandCount = copyRootFilesMatching(
    sourcePath,
    targetPath,
    /\.jay-command$/,
  );
...
function copyPackage(packageName, sourcePath) {
  ...
  try {
    ...
    console.log(
      `${colors.red}  ✗ Error copying ${packageName}: ${error.message}${colors.reset}`,
    );
    return false;
  }
}
...
function main() {
  if (!fs.existsSync(SOURCE_REPO)) {
    console.log(
      `${colors.red}Error: Source repository not found at ${SOURCE_REPO}${colors.reset}`,
    );
    process.exit(1);
  }
  ...
  if (!fs.existsSync(jayFrameworkDir)) {
    console.log(
      `${colors.yellow}No @jay-framework directory found in node_modules${colors.reset}`,
    );
    console.log(
  ...
  const installedPackages = fs
    ...
    .filter((entry) => {
      ...
      return fs
        .statSync(path.join(jayFrameworkDir, entry.name))
        .isDirectory();
    })
  ...
  if (installedPackages.length === 0) {
    console.log(
      `${colors.yellow}No @jay-framework packages found in node_modules${colors.reset}`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `${colors.green}✓ ${successCount} packages copied successfully${colors.reset}`,
  );
  if (failCount > 0) {
