...
function main() {
  ...
  if (!fs.existsSync(jayFrameworkDir)) {
    ...
    console.log(
      'Run npm install first to install the packages, then run this script to sync them.',
    );
    process.exit(0);
  }
  ...
  console.log(
    '2. Check that the package names in your package.json match the actual package structure',
  );
}
