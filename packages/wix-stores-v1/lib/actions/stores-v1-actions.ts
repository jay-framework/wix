    ...
    .withHandler(
        async (
            ...
            try {
                ...
                const minBound =
                    minPriceResult.items?.[0]?.price?.price ??
                    minPriceResult.items?.[0]?.priceData?.price ??
                    0;
                const maxBound =
                    maxPriceResult.items?.[0]?.price?.price ??
                    maxPriceResult.items?.[0]?.priceData?.price ??
                    0;

                // Get currency symbol from first product
                const currency =
                    products[0]?.price?.currency ||
                    products[0]?.priceData?.currency ||
                    minPriceResult.items?.[0]?.price?.currency;
    ...
    .withHandler(
        async (
            ...
            try {
                ...
                let result = await wixStores.products
                    .queryProducts()
                    .eq('slug', slug)
                    .limit(1)
                    .find();
                ...
                if (!result.items?.length) {
                    result = await wixStores.products
                        .queryProducts()
                        .eq('_id', slug)
                        .limit(1)
                        .find();
