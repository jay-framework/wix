...
function generateEntrySource(
    ...
    const pluginsArray = plugins.map(
        (p, i) => `    { name: '${p.name}', init: pluginModule_${i}.init },`,
    );
...
// Plugin imports (full modules for init + page-parts resolution)
${moduleImports.join('\n')}
...
function generateServeSource(frontendDir: string, backendDir: string): string {
    const lines = [
        ...
        '    let fp = path.join(FRONTEND_DIR, pathname);',
        '    if (!fs.existsSync(fp) || !fs.statSync(fp).isFile()) {',
        '        fp = path.join(FRONTEND_DIR, "public", pathname);',
