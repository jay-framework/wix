...
function writeGeneratedAddMenuCatalog(
    ...
    fs.mkdirSync(path.dirname(outputPath), { recursive: true });
    const header = '# DO NOT EDIT BY HAND — regenerated by jay-stack agent-kit (wix-stores)\n\n';
...
#
# To see available categories: jay-stack agent-kit (generates category tree reference)
...
export async function setupWixStores(ctx: PluginSetupContext): Promise<PluginSetupResult> {
    ...
    return {
        status: 'configured',
        message: `Wix Stores configured (product URL: ${service.urls.product})`,
        ...(configCreated.length > 0 ? { configCreated } : {}),
    };
}

// ============================================================================
// Agent-kit handler (jay-stack agent-kit) — references + add-menu catalogs
// ============================================================================
...
export async function generateWixStoresAgentKit(
    ...
    agentKitCreated.push(`agent-kit/references/${ctx.pluginName}/categories.yaml`);

    const addMenuGenerated = writeGeneratedAddMenuCatalog(ctx.projectRoot, roots);
    agentKitCreated.push(addMenuGenerated);
