...
async function* loadItemParams([wixData, contract]: [WixDataService, Contract]): AsyncIterable<
  ItemPageParams[]
> {
...
async function fetchReference(refValue: any, wixData: WixDataService): Promise<any> {
  ...
  if (Array.isArray(refValue)) {
    return Promise.all(
      refValue.map((id) => wixData.items.getDataItem(id).then((r) => r.dataItem?.data)),
    );
  }
...
export function provideWixDataService(wixClient: WixClient, config: WixDataConfig): WixDataService {
...
<section class="blog-posts">
  ...
  <button ref="posts.loadMoreButton" when="hasMore" is="true">Load More</button>
</section>
```
