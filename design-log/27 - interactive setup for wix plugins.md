...
Jay Framework DL#157 introduces interactive plugin setup — setup handlers can now prompt the user for input via `ctx.prompt.input()`, `ctx.prompt.confirm()`, etc. This replaces the current approach where `create-jay` hardcodes Wix-specific logic (API key prompts, `@wix/cli login`, `wix.config.json` parsing).
...
2. Check Wix CLI login: `ctx.run('npx @wix/cli whoami')` — if fails, run `ctx.run('npx @wix/cli login')`
3. Check `wix.config.json` exists — if not, run `ctx.run('npm create @wix/new@latest init')`
4. Read `appId` and `siteId` from `wix.config.json`
5. Prompt for API key: `ctx.prompt.input({ message: 'Wix API Key (create at https://manage.wix.com/account/api-keys):' })`
6. Write `config/.wix.yaml` with real values (not placeholders)
7. Update `.gitignore` — add `config/.wix.yaml` and `wix.config.json`

**Non-interactive fallback:** Create template with `<placeholder>` values (current behavior), return `needs-config`.
...
export async function setupWixServerClient(ctx: PluginSetupContext): Promise<PluginSetupResult> {
    ...
    // Non-interactive: create template and bail
    if (!ctx.interactive) {
        if (!fs.existsSync(configPath)) {
            fs.mkdirSync(ctx.configDir, { recursive: true });
            fs.writeFileSync(configPath, CONFIG_TEMPLATE, 'utf-8');
            return { status: 'needs-config', configCreated: ['config/.wix.yaml'], message: 'Fill in credentials and re-run setup' };
        }
        return { status: 'needs-config', message: 'Config has placeholder values' };
    }

    // Interactive: full credential flow
    // 1. Wix CLI login
    try {
        ctx.run('npx @wix/cli whoami');
    } catch {
        ctx.log('You need to log in to Wix');
        ctx.run('npx @wix/cli login');
    }

    // 2. Connect to site
    const wixConfigPath = path.join(ctx.projectRoot, 'wix.config.json');
    if (!fs.existsSync(wixConfigPath)) {
        ctx.log('Connecting to your Wix site...');
        ctx.run('npm create @wix/new@latest init');
    }
...
Already auto-fills `clientId`/`siteId` from `wix.config.json` and auto-creates the `jay-backend-files` data collection if missing. No user prompts needed — once wix-server-client has credentials, everything is automated.
...
### 3. wix-members — Create auth callback page interactively
...
4. If missing and `ctx.interactive`: prompt with `ctx.prompt.confirm()`, create from template if confirmed
5. If missing and non-interactive: return `needs-config` (current behavior)
    ...
    if (!fs.existsSync(fullPath)) {
        if (ctx.interactive) {
            ...
            if (create) {
                ...
                ctx.log(`Created ${expectedPath}`);
            } else {
                return { status: 'needs-config', configCreated, message: `Auth callback page missing: ${expectedPath}` };
            }
        } else {
            return { status: 'needs-config', configCreated, message: `Auth callback page missing: create ${expectedPath}` };
        }
    }
...
These plugins validate API access and generate references. No user prompts needed:
...
### Phase 1: Wait for Jay DL#157

The interactive setup context (`ctx.prompt`, `ctx.interactive`, `ctx.run`) must be implemented in the framework first.

### Phase 2: Update wix-server-client
...
3. Port `.gitignore` update logic from `create-jay`

### Phase 3: Update wix-members

1. Move `create-jay/templates/auth-callback.jay-html` template into the wix-members package
2. Update `setupWixMembers` in `packages/wix-members/lib/setup.ts` with interactive callback page creation

### Phase 4: Remove from create-jay

1. Remove `promptWixApiKey`, `setupWix`, `hasWixPlugins` from `create-jay/lib/index.ts`
2. `create-jay` just runs `jay-stack-cli setup` after install (interactive by default)

## Verification Criteria

1. Fresh `npm create jay` with Wix plugins prompts for API key via the plugin's setup handler (not create-jay)
2. `jay-stack-cli setup` on an existing project re-runs the Wix credential flow if not configured
3. `jay-stack-cli setup --no-interactive` creates config templates without prompting
4. Auth callback page is created by wix-members setup, not create-jay
5. wix-deploy auto-creates `jay-backend-files` collection without prompts
6. Plugins without interactive needs (wix-stores, wix-data, wix-media) continue to work unchanged