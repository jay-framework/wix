...
A: Try to reuse the same contracts - they define _view state_, not API response. The product mapper converts V1 responses to ViewState. If V1 data doesn't fit the contracts well, create similar but V1-specific contracts. **Verify during implementation.**
